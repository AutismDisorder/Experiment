...
class MutationOperator:
    ...
    def _point_mutation(self, gene: CapabilityGene, drives: Dict[str, float]) -> Optional[CapabilityGene]:
        ...
        child = CapabilityGene(
            ...
            code_hash=hashlib.md5(f"{gene.code_hash}_mut_{datetime.now(timezone.utc).timestamp()}".encode()).hexdigest()[:16],
        ...
        child.mutations.append({'type': 'point', 'timestamp': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')})
        return child
    ...
    def _structural_mutation(self, gene: CapabilityGene, drives: Dict[str, float]) -> Optional[CapabilityGene]:
        ...
        child.mutations.append({'type': 'structural', 'added': new_func, 'timestamp': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')})
        return child
    ...
    def _drive_affinity_mutation(self, gene: CapabilityGene, drives: Dict[str, float]) -> Optional[CapabilityGene]:
        ...
        child.mutations.append({'type': 'drive_affinity_shift', 'toward': dominant, 'timestamp': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')})
        return child
    ...
    def _signature_mutation(self, gene: CapabilityGene, drives: Dict[str, float]) -> Optional[CapabilityGene]:
        ...
        child.mutations.append({'type': 'signature_change', 'reframed_by': dominant, 'timestamp': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')})
        return child
    ...
    def _meta_rule_mutation(self, gene: CapabilityGene, drives: Dict[str, float]) -> Optional[CapabilityGene]:
        ...
        child = CapabilityGene(
            ...
            source_path=gene.source_path,
            code_hash=hashlib.md5(f"{gene.code_hash}_meta_{datetime.now(timezone.utc).timestamp()}".encode()).hexdigest()[:16],
        ...
        child.mutations.append({'type': 'meta_rule_injection', 'timestamp': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')})
        return child
...
class CrossoverOperator:
    ...
    def _function_merge_crossover(self, p1: CapabilityGene, p2: CapabilityGene, 
        ...
        child.crossovers.append({'type': 'function_merge', 'parents': [p1.gene_id, p2.gene_id], 'timestamp': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')})
        return child
    ...
    def _drive_fusion_crossover(self, p1: CapabilityGene, p2: CapabilityGene,
        ...
        child.crossovers.append({'type': 'drive_fusion', 'dominant': dominant, 'parents': [p1.gene_id, p2.gene_id], 'timestamp': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')})
        return child
    ...
    def _pipeline_crossover(self, p1: CapabilityGene, p2: CapabilityGene,
        ...
        child.crossovers.append({'type': 'pipeline', 'parents': [p1.gene_id, p2.gene_id], 'timestamp': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')})
        return child
    ...
    def _synthesis_crossover(self, p1: CapabilityGene, p2: CapabilityGene,
        ...
        child.crossovers.append({'type': 'synthesis', 'meta_parent': meta_parent.gene_id, 'other_parent': other_parent.gene_id, 'timestamp': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')})
        return child
