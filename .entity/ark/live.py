...
def load_state(workdir):
    ...
    state["identity"] = f"ark-{os.getpid()}-{int(datetime.now(timezone.utc).timestamp())}"
...
def register_member(workdir, manifest, identity, role, dirname, drives):
    ...
    manifest["members"].append({
        "identity": identity,
        "role": role,
        "born": datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z'),
        "dir": dirname,
...
def birth(workdir, state):
    ...
    child["born"] = datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')
...
def main():
    ...
    for i in range(iterations):
        ...
        if veto:
            ...
            write_note(workdir, {"by": state["identity"], "iteration": it, "kind": "lesson", "note": f"edge held: {veto[0]} — this edge is a colony lesson, born abroad", "ts": datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')})
        else:
            note = ACTIONS[chosen]
        ...
        if chosen == "build":
            ...
            write_note(workdir, {"by": state["identity"], "iteration": it, "kind": "build", "note": f"forged {artifact}", "ts": datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')})
        elif chosen == "explore":
            n = len(list(workdir.iterdir()))
            state["memory"]["observations"].append(f"terrain: {n} files at iteration {it}")
            write_note(workdir, {"by": state["identity"], "iteration": it, "kind": "explore", "note": f"terrain observed: {n} files", "ts": datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')})
